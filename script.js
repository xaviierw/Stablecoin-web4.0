...
function showLoadingScreen(stablecoinInput) {
    ...
    setTimeout(() => {
        ...
        try {
                ...
                datasets: [
                    {
                        ...
                        data: [1.2, 1.3, 1.25, 1.4, 1.35, 1.5] // Values in billions
                    },
                    {
                        label: 'Minted on Tron',
                        backgroundColor: 'rgba(255, 99, 132, 0.2)',
                        borderColor: 'rgba(255, 99, 132, 1)',
                        data: [1.1, 1.15, 1.2, 1.25, 1.3, 1.35] // Values in billions
                    },
                    {
            ...
            const ethMinted = selectedData.minted.ethereum.minted.match(/\d+/g).join('');
            const tronMinted = selectedData.minted.tron.minted.match(/\d+/g).join('');
            ...
            chartInstance.data.datasets[0].data = Array(6).fill(Number(ethMinted) / 1e9); // Example: 30B USDT -> 30
            chartInstance.data.datasets[1].data = Array(6).fill(Number(tronMinted) / 1e9); // Example: 10B USDT -> 10
            chartInstance.data.datasets[2].data = Array(6).fill(Number(totalCollateral) / 1e9); // Example: 40B USDT -> 40
            chartInstance.data.datasets[3].data = Array(6).fill(Number(totalMinted) / 1e9); // Example: 40B USDT -> 40
